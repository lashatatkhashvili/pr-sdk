...
const Promofy = (() => {
  ...
  let onTokenExpiringCallback = null; // Set via configure({ onTokenExpiring })
  ...
  window.addEventListener("message", (event) => {
    ...
    // Widget token-expiry warning (new event-style messages)
    if (data.event === "ON_TOKEN_EXPIRY_WARNING") {
      if (typeof onTokenExpiringCallback === "function") {
        onTokenExpiringCallback(data.data ?? {});
      }
      return;
    }

    // Legacy action-style messages
  ...
  /**
   * Called by the client after obtaining a fresh session token via the
   * onTokenExpiring callback. Sends { event: "TOKEN_REFRESH", data: { token } }
   * to every active iframe so the widget's AuthProvider can re-authenticate
   * silently — no reload, no flicker.
   */
  const refreshToken = (token) => {
    const targetOrigin = baseWidgetUrl ? new URL(baseWidgetUrl).origin : "*";
  ...
  const configure = (config = {}) => {
    ...
    if (typeof config.onTokenExpiring === "function") {
      onTokenExpiringCallback = config.onTokenExpiring;
    }
